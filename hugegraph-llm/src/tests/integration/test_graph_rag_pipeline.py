...
class RAGPipeline:
    ...
    def query_graphdb(self, max_deep=2, max_graph_items=10, max_v_prop_len=2048, max_e_prop_len=256, 
                     prop_to_match=None, num_gremlin_generate_example=1, gremlin_prompt=None):
    ...
    def merge_dedup_rerank(self, graph_ratio=0.5, rerank_method="bleu", near_neighbor_first=False, custom_related_information=""):
        if "merge_dedup_rerank" in self.operators:
            return self.operators["merge_dedup_rerank"]({"graph_result": [], "vector_result": []})
        return {"merged_result": []}
    
    def synthesize_answer(self, raw_answer=False, vector_only_answer=True, graph_only_answer=False, 
                         graph_vector_answer=False, answer_prompt=None):
    ...
    def run(self, **kwargs):
        ...
        if not kwargs.get("skip_synthesize_answer", False):
            context.update(self.synthesize_answer(
