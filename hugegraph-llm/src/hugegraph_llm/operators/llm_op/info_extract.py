...
def extract_data(data: str) -> dict:
    ...
    # Extract the content inside "result"
    result_content = re.search(r'"result":\s*"(.*?)"', result, re.DOTALL).group(1)

    # Replace escaped double quotes with double quotes
    result_content = result_content.replace('\\"', '"')

    # Extract the content for nodes, relationships, nodes_schemas, and relationships_schemas
    nodes = re.findall(r'\[\s*".*?\s*\]',
                       re.search(r'Nodes:\s*\n*\n*(.*?)\n*\n*Relationships:', result_content,
                                 re.DOTALL).group(1))

    # match = re.search(r'Nodes:\s*\n*\n*(\[\s*".*?\s*])', result_content, re.DOTALL)
    # if match is not None:
    #     nodes = match.group(1)
    # else:
    #     print("No match found")
    relationships = re.search(r'Relationships:\s*\n*\n*(\[\s*{.*?}\s*])', result_content,
                              re.DOTALL).group(1)
    nodes_schemas = re.search(r'NodesSchemas:\s*\n*\n*(\[\s*".*?\s*])', result_content,
                              re.DOTALL).group(1)
    relationships_schemas = re.search(r'RelationshipsSchemas:\s*\n*\n*(\[\s*".*?\s*])',
                                      result_content, re.DOTALL).group(1)

    # Convert the extracted content to JSON
    nodes = json.loads(nodes)
    relationships = json.loads(relationships)
    nodes_schemas = json.loads(nodes_schemas)
    relationships_schemas = json.loads(relationships_schemas)

    return {
        "nodes": nodes,
        "relationships": relationships,
        "nodes_schemas": nodes_schemas,
