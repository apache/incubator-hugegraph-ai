...
class BuildVectorIndex:
    def __init__(self, embedding: BaseEmbedding):
        ...
        self.file_prefix = get_filename_prefix(
            llm_settings.embedding_type,
            getattr(self.embedding, "model_name", None)
        )
        self.vector_index = VectorIndex.from_index_file(self.index_dir, self.file_prefix)
    ...
    def run(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if len(chunks_embedding) > 0:
            ...
            self.vector_index.to_index_file(self.index_dir, self.file_prefix)
        return context
