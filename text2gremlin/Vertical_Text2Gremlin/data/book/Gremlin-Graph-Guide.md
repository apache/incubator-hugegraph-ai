...
### Loading a graphML file TP2 vs TP3 {#ld2}
...
## A word about indexes and schemas {#indexschema}
...
### Counting groups of things {#countgroup}
...
### Modifying a *path* using *from* and *to* modulators {#pathfromto}
...
### Does an edge exist between two vertices? {#edgeexist}
...
### Using multiple *as* steps with the same label {#multias}

It is actually possible using an *as* step to give more than one part of
...
### Returning selected parts of a path {#pathselect}

Sometimes, even using the *from* and *to* modulating steps along with a
...
### Examining the edge between two vertices {#exedge}

Sometimes, it is the edge between two vertices that we are interested in
...
## Limiting the amount of data returned {#limit}

It is sometimes useful, especially when dealing with large graphs, to
...
### Retrieving a range of vertices {#retrrange}
...
### Removing duplicates - introducing *dedup* {#dedup}

It is often desirable to remove duplicate values from query results. The
...
## Using *valueMap* to explore the properties of a vertex or edge {#vm}

A call to *valueMap* will return all of the properties of a vertex or
...
### Changes to *valueMap* introduced in TinkerPop 3.4 {#tp34vm}

Starting with the Apache TinkerPop 3.4 release, a few changes have been
...
## An alternative to *valueMap* - introducing *elementMap* {#element-map}

A new step, *elementMap*, was added to the Gremlin language as part of
...
## Assigning query results to a variable {#var}

It is extremely useful to be able to assign the results of a query to a
...
### Introducing *toList*, *toSet*, *bulkSet* and *fill* {#setsandlists}

It is often useful to return the results of a query as a list or as a
...
## Working with IDs {#wid}

Every vertex, every edge and even every property in a graph has a unique
...
## Working with labels {#lab}

It's a good idea when designing a graph to give the vertices and edges
...
## Using the *local* step to make sure we get the result we intended {#local}

Sometimes it is important to be able to do calculations based on the
...
## Basic statistical and numerical operations {#st}

The following queries demonstrate concepts such as calculating the
...
## Testing values and ranges of values {#tranges}

We have already seen some ways of testing whether a value is within a
...
### Using *between* to simulate *startsWith* {#startswith}

One thing that may not be obvious is that when using string values with
...
### Refining flight routes analysis using *not*, *neq*, *within* and *without* {#winout}

As we saw in the previous section, it is often useful to be able to
...
### Using *coin* and *sample* to sample a dataset {#_using_coin_and_sample_to_sample_a_dataset}

In any sort of analysis work it is often useful to be able to take a
...
### Using *Math.random* to more randomly select a single vertex {#_using_math_random_to_more_randomly_select_a_single_vertex}

While the *sample* step allows you to select one or more vertices at
...
## New text search predicates added in TinkerPop 3.4 {#textpredicates}

Probably one of, if not the, most anticipated features in Apache
...
The example below looks for any city names ending with that characters
\"zhi\".
...
We can also look for cities whose names contain a certain string of one
...
Using *notEndingWith* we can easily find cities whose names do not end
with \"zhi\".
...
The query below counts the number of cities that do not contain the
...
## Sorting things - introducing *order* {#sort}

You can use *order* to sort things in either ascending (the default) or
...
### Sorting by key or value {#sortkeyvalue}

Sometimes, when the results of a query are a set of one or more
...
### Changes to *order* introduced in TinkerPop release 3.3.4 {#orderchanges}

On October 15th 2018 a change was introduced as part of the Apache
...
Gremlin provides a set of logical operators such as *and*, *or* and
...
## Using *where* to filter things out of a result {#where}

We have already seen the *where* step used in some of the prior
...
### Using *where* and *by* to filter results {#whereby}

A new capability was added in the Tinkerpop 3.2.4 release that allows a
...
## Using *choose* to write if...​then...​else type queries {#choose}

The *choose* step allows the creation of queries that are a lot like the
...
### Including a constant value - introducing *constant* {#chooseconstant}

Sometimes it is very useful, as the example below demonstrates, to
...
## Using *option* to write case/switch type queries {#option}

When *option* is combined with *choose* the result is similar to the
...
## Using *match* to do pattern matching {#patmatch}

The *match* step was added in TinkerPop 3 and allows a more declarative
...
### Pattern matching using a *where* step {#patternwhere}

As the Gremlin language has evolved, many queries that might seem a
...
## Using *union* to combine query results {#union}

The *union* step works just as you would expect from its name. It allows
...
### Introducing the *identity* step {#unionidentity}

Gremlin has an *identity* step that we have not seen used so far in this
...
### Using *constant* values as part of a *union* {#unionconstant}

We have already seen the *constant* step used in the \"[Including a
...
### More examples of the *union* step {#unionmore}

The following query uses a *sample* step to select 10 airports at random
...
### Using *union* to combine more complex traversal results {#unionthree}

So far the examples we have looked at mostly show fairly simple
...
## Using *sideEffect* to do things on the side {#sideeffect}

The *sideEffect* step allows you to do some additional processing as
...
## Using *aggregate* to create a temporary collection {#aggregate}

At the time of writing this book, there were 59 places you could fly to
...
## Using *inject* to insert values into a query {#inject}

Sometimes you may want to add something additional to be returned along
...
### A useful trick using *inject* {#injecttrick}

There is also a useful trick that can be achieved using *inject*. Take a
...
## Using *coalesce* to see which traversal returns a result {#coalesce}

Sometimes, when you are uncertain as to which traversal of a set you are
...
### Combining *coalesce* with a *constant* value {#coalconst}

The *coalesce* step can also be very useful when combined with a
...
## Returning one of two possible results - introducing *optional* {#optional}

Sometimes it may be useful to return one of two results depending upon
...
## Other ways to explore vertices and edges using *both*, *bothE*, *bothV* and *otherV* {#otherv}

We have already looked at examples of how you can walk a graph and
...
## Shortest paths (between airports) - introducing *repeat* {#sp}
...
### Using *emit* to return results during a *repeat* loop {#emit}

Sometimes it is useful to be able to return the results of a traversal
...
### Nested and named *repeat* steps {#nestedrepeat}

Starting with Apache TinkerPop release 3.4 it is now possible to nest a
...
### Limiting the results at each depth {#depthlimit}

Sometimes, while exploring a graph using *repeat* steps, it is desirable
...
### Haven't I been here before? - Introducing *cyclicPath* {#cyclicpath}

You can use the *cyclicPath* step to find paths through the graph that
...
### A warning that path finding can be memory and CPU intensive {#pathwarn}

Take a look at the query below. It returns the first 10 routes found
...
### A warning that the *path* and *as* steps can also be memory intensive {#pathstepwarn}

The Gremlin *path* step is incredibly useful and I find myself using it
...
## Calculating vertex degree {#nd}

While working with graphs, the word *degree* is used when discussing the
...
## Gremlin's scientific calculator - introducing *math* {#mathstep}

As we have seen in some of the prior sections, there are some Gremlin
...
### Performing simple arithmetic {#arithmentic}

Let's start by looking at a few basic examples. First of all the query
...
### Using a *by* modulator with a *math* step {#mathby}

As with many Gremlin steps, a *math* step can also be combined with one
...
### Converting feet to meters {#mathconvert}

The length of the longest runway at each airport in the graph is stored
...
### Using the trigonometric functions {#mathtrig}

The trigonometric operators work as you would expect. All angles need to
...
### Using signum to make a choice {#mathsignum}

The *signum* operator allows us to make a decision depending upon
...
### Calculating a standard deviation {#stddev}

We could use the new *math* step to implement a query that calculates
...
### Calculating a standard deviation in one query {#stddevone}

In the previous section the steps to calculate the standard deviation
...
### Using *project* to feed values to *math* {#mathproject}

The *project* step can be used to create a map of key/value pairs that
...
## Including an index with results - introducing *withIndex* and *indexed* {#withindex}

If for any reason you wanted an index value included as part of the
...
### The new *index* step added in TinkerPop 3.4 {#tp34index}

Apache TinkerPop version 3.4, released at the start of 2019, added a
...
### Using *index* to reverse a list {#index-reverse}

The Gremlin query language does not have a built in step or function
...
## More examples using concepts we have covered so far {#sx}

The examples in this section build upon the topics that we have covered
...
# BEYOND BASIC QUERIES {#beq}

So far we have looked mostly at querying an existing graph. In the
...
## A word about layout and indentation {#indents}

As you begin to write more complex Gremlin queries they can get quite
...
## A warning about reserved word conflicts and collisions {#rword}

Most of the time the issue I am about to describe will not be a problem.
...
## Thinking about your data model {#dmodel}
...
### Keeping information in two places within the same graph {#_keeping_information_in_two_places_within_the_same_graph}

Sometimes, to improve query efficiency I find it is actually worth
...
### Using a graph as an index into other data sources {#_using_a_graph_as_an_index_into_other_data_sources}

While on the topic of what to keep in the graph, something to resist
...
### A few words about *supernodes* {#_a_few_words_about_supernodes}

When a vertex in a graph has a large number of edges and is
...
## Making Gremlin even Groovier {#grv}

As we have already discussed, the Gremlin console builds upon the Groovy
...
### Using a variable to feed a traversal {#varaus}

Sometimes it is very useful to store the result of a query in a variable
...
## Adding vertices, edges and properties {#addnodes}

So far in this book we have largely focussed on loading a graph from a
...
### Adding an airport (vertex) and a route (edge) {#_adding_an_airport_vertex_and_a_route_edge}

The following code uses the *graph* object that we created when we first
...
In TinkerPop 3.3.1 a new capability was added to the *addV* and *addE*
...
### Using a traversal to seed a property with a list {#proptraversal}

You can use the results of a traversal to create or update properties.
...
### Using *inject* to specify new vertex ID values {#injectid}

If the graph database you are using supports user provided ID values,
...
### Quickly building a graph for testing {#testgraph}

Sometimes for testing and for when you want to report a problem or ask
...
### Adding vertices and edges using a loop {#addloop}

Sometimes it is more efficient to define the details of the vertices or
...
### Using *coalesce* to only add a vertex if it does not exist {#coaladdv}

In the [Combining with a value](#coalconst) section we looked at how
...
### Using *coalesce* to derive an *upsert* pattern {#upsert}

Using *coalesce* in this way provides us with a nice pattern for a
...
### Creating one vertex based on another {#vertexcopy}

It is sometimes useful to be able to create a new vertex using the label
...
## Deleting vertices, edges and properties {#deleting}

So far in this book we have looked at several examples where we created
...
### Deleting a vertex {#_deleting_a_vertex}

In some of our earlier examples we created a fictitious airport vertex
...
### Deleting an edge {#deledge}

We can also use *drop* to remove specific edges. The following code will
...
Lastly, we can use *drop* to delete a specific property value from a
...
### Removing all the edges or vertices in the graph {#_removing_all_the_edges_or_vertices_in_the_graph}

This may not be something you want to do very often, but should you wish
...
## Property keys and values revisited {#pkvrevisited}

We have already looked, earlier in the book, at numerous queries that
...
### The *Property* and *VertexProperty* interfaces {#vertexprop}

In a TinkerPop 3 enabled graph, all properties are implementations of
...
### The *propertyMap* traversal step {#propmap}

We have previously used the *valueMap* step to produce a map of
...
### Properties have IDs too {#propid}

We have seen many examples already that show how both vertices and edges
...
### Attaching multiple values (lists or sets) to a single property {#listprop}

A vertex property value can be a basic type such as a String or an
...
### A word of caution - behavior differences with *property* {#propertycaution}
...
### What did Gremlin do? - introducing *explain* {#explainstep}

If you ever want to know how Gremlin compiles your query into a form
...
### Updating properties stored in a list {#updatelist}

So now we know how to create a vertex with property values in a list we
...
### Creating properties that store sets {#propsets}

So far we have just created values in a list that have *LIST*
...
### One more note about sets and lists {#setlistnote}

Note that the other examples we have shown in this section are not the
...
### Adding properties to other properties (meta properties) {#metaprop}

TinkerPop 3 introduced the ability to add a property to another
...
### Using *unfold* and *WithOptions* with Meta Properties {#tp34vmmetaprop}

A new feature was introduced in Apache TinkerPop version 3.4 that allows
...
## Deducing the schema of a graph using queries {#deduce}

Sometimes, you may find yourself working with a graph while being unsure
...
## Collections revisited {#collrev}

As we have seen in many of the prior examples, very often, either in the
...
### Steps that generate collections {#collsteps}

Let's start this discussion by first reviewing a few ways that a
...
### Accessing the contents of a collection {#collaccess}
...
### Using *unfold* to unbundle a collection {#unbundle}

Sometimes it is desirable to unbundle a collection so that we can work
...
### Using *local* scope with collections {#localcollect}

Sometimes if you want to work on the contents of a collection, whether
...
## Collections and reducing barrier steps {#rbarriers}

If you look at commonly asked questions about writing Gremlin queries on
...
### Calculating the *sum* of a collection {#sumcollection}

The query below returns a map where the keys are airport IATA codes and
...
### Using the *math* step with collections {#mathcollection}

Building on the examples from the previous section, let's now take these
...
## Introducing *sack* as a way to store values {#sackintro}

As is hopefully now becoming apparent, to efficiently write certain
...
### Basic *sack* operations {#sackbasics}

The *sack* step, as shown in the examples below, is a side effect step,
...
### Using *min* and *max* with a *sack* {#sackminmax}

There are many different operators that can be used with sacks. They are
...
### Doing calculations using a *sack* {#sackcalc}

Now let's look at a more complex, and hopefully more interesting,
...
### Doing calculations without using a *sack* {#nonsackcalc}

A similar result to those from the queries in the previous section can
...
### Computing hop counts using a *sack* {#sackpathlength}

The next query we are going to look at uses a *sack* to keep track of
...
### Using boolean operators with a *sack* {#sackbooleans}

You can also use the boolean operators *or* and *and* when working with
...
### Using *addAll* and lists with a *sack* {#sackaddall}

So far we have looked at using numbers and boolean values with a sack.
...
### Comparing properties and constants to the value of a sack {#sackpredicate}

During a traversal you may want to incrementally modify a sack and then
...
## Using Lambda functions {#lambdas}

Gremlin allows you to include a code fragment (sometimes called a Lambda
...
### Introducing the *Map* step {#mapstep}

The *map* step will be familiar to users of programming languages such
...
### Using lambdas with *sack* steps {#lambdasack}

In the \"[Using and lists with a ](#sackaddall)\" section we introduced
...
### Introducing the *flatMap* step {#flatmap}

There are a set of fundamental steps that the other Gremlin query
...
### Using regular expressions to do fuzzy searches {#fuzzyregs}

Let's take a look at one case where use of closures might be helpful. It
...
## Creating custom tests (predicates) {#pred}

TinkerPop comes with a set of built in methods that can be used for
...
### Creating a regular expression predicate {#_creating_a_regular_expression_predicate}

In the previous section we used a closure to filter values using a
...
## Unrolling the lists returned by *valueMap* {#vmunroll}

In the \"[Changes to introduced in TinkerPop 3.4](#tp34vm)\" section, I
...
## Using graph variables to associate metadata with a graph {#graphvars}

TinkerPop 3 introduced the concept of graph variables. A graph variable
...
## Turning graphs into trees {#tre}

TinkerPop defines a Tree API but it is not that well fleshed out and has
...
## Creating a sub graph {#subgraph}

Using Gremlin, you can create a subgraph which is a subset of the
...
## Working with GraphML and GraphSON {#graphmlandjsonintro}

Apache TinkerPop supports the loading and saving of entire graphs using
...
### Saving (serializing) a graph as GraphML (XML) or GraphSON (JSON) {#sav}

Using TinkerPop 3 you can save a graph either in GraphML or Graphson
...
### Loading a graph stored as GraphML (XML) or GraphSON (JSON) {#reload}

In section 2 we saw how to load the `air-routes.graphml` file. In case
...
### Turning the results of a query into JSON {#graphsonmapper}

In the sections above we explored how to save and load an entire graph
...
## Analyzing the performance of your queries {#performance}

Apache TinkerPop includes a class called TimeUtil that provides methods
...
### Timing a query - introducing *clock* and *clockWithResult* {#clock}

The *TimeUtil* class provides several methods that can be used when
...
### Analyzing where time is spent - introducing *profile* {#profile}

You can use the *profile* step to ask Gremlin to give you a more fine
...
### Introducing TinkerGraph indexes {#tinkerindex}

TinkerGraph provides a rudimentary indexing capability but using it can
...
## OLTP vs OLAP {#olapoltp}

When discussing graph processing two terms regularly come up. These
...
### Introducing the TinkerPop Graph Computer {#graphcomputer}

A lot of new capability was added to the TinkerPop framework as part of
...
### Experiments with Page Rank {#pagerank}

Probably one of, if not the, most famous algorithms in the big data
...
# MISCELLANEOUS QUERIES AND THEIR RESULTS {#msc}

In this chapter you will find more Gremlin queries that operate on the
...
## Counting more things {#countmore}

To get things started, let's look at a few more examples that basically
...
### Which countries have no airports? {#noairports}

This first query looks for any *country* vertices that have no outgoing
...
### Which airports have no routes? {#noroutes}

There are a few airports in the graph that currently have no commercial
...
### What is the distribution of runways? {#runwaydist}

We can easily count the distribution per airport of runways. We can
...
### Airports with the most routes {#mostroutes}

This next query finds all airports that have more than 180 outgoing
...
### Airports with just one route {#oneroute}

There are, perhaps surprisingly, a large number of airports that only
...
### Single runway airports with the most routes {#onerunway}

It is interesting to look at how busy some single runway airports are.
...
### Another way of counting runways {#runwaycountproject}

Let's assume we wanted to count the runways in the graph and for each
...
### Airports with the most routes in Canada {#canadamost}

The following query finds the top 10 airports in Canada sorted by
...
### Distribution of UK airports {#ukdist}

How many airports are there in each of the UK regions of England,
...
### Distribution of airports by country {#countrydist}

This query uses *groupCount* to produce a map of key value pairs where
...
### Distribution of airports by continent {#continentdist}

We can use a similar query to the one above to find out how many
...
### Distribution of routes per airport {#_distribution_of_routes_per_airport}

We have already examined various ways to calculate the distribution of
...
### Using groupCount with a traversal variable {#groupvar}

So far we have just used *groupCount* with no parameters. When used in
...
### Combining *groupCount* and *constant* {#gcconstant}

You can use a *constant* value in combination with a *groupCount* step
...
### Combining *choose* and *groupCount* {#chainchoose}

Using the ideas discussed above, we can write a query that chains
...
### Nesting one *group* step inside another {#nestedgroup}

Many examples of both the *groupCount* and *group* steps have already
...
### Analysis of routes between Europe and the USA {#eu-usa}

The next few queries show how you can use a graph like *air-routes* to
...
### Using *fold* to do simple Map-Reduce computations {#mapreduce}
...
### Distribution of routes in the graph (mode and mean) {#meanmode}

An example of a common question we might want to answer with a network
...
### How many routes are there from airports in London (UK)? {#_how_many_routes_are_there_from_airports_in_london_uk}

This next query can be used to figure out how many outgoing routes each
...
### How many routes are there between airports in England? {#englandroutes}

We can use an *aggregate* step to quite elegantly find the routes that
...
### What are the top ten airports by route count? {#topten}

Earlier we calculated which airports have the most routes. These next
...
### Using *local* while counting things {#localfold}

In some of the earlier sections we saw examples of *local* scope being
...
### How many vertices have no edges? {#noedges}

In the air routes graph there are some vertices that have no outgoing
...
## Where can I fly to from here? {#wherefly}

In this section you will find some more examples of queries that explore
different questions along the lines of \"Where can I fly to from
here?\".

### Does any route exist between two airports? {#doesanyrouteexist}

We have already looked at different ways to discover shortest routes (by
...
### Where in the USA or Canada can I fly to from any airport in India? {#uscatoindia}

This first query looks for routes to the USA or Canada from any airport
in India.
...
### Which cities in Europe can I fly to from Ft. Lauderdale in Florida? {#flleurope}

This query looks for routes from Fort Lauderdale in Florida to cities in
Europe.
...
### Where can I fly to from Charlotte, to cities in Europe or South America? {#clteusa}

This query looks for routes from Charlotte, North Carolina to cities in
...
### Where in the United States can I fly from to airports in London? {#usalon}
...
### Where in New York state can I fly to from any of the airports in Texas? {#txny}

This query looks for all the routes from any airport in Texas that end
...
### Which cities in Mexico can I fly to from Denver? {#denmex}
...
### Which cities in Europe can I fly to from Delhi in India? {#deleu}
...
### Finding all routes between London, Munich and Paris {#aggrroutes}

In the following example we find all the routes between airports in
...
## More analysis of distances between airports {#moredist}

In this section you will find some more queries that examine distances
...
### Finding routes longer than 8,000 miles {#gt8k}

This next set of queries show various ways of finding and presenting all
...
### Finding the 20 longest routes in the graph {#_finding_the_20_longest_routes_in_the_graph}

We could write the query from the previous section a different way
...
### Finding the longest route from each airport {#longesteach}

The following query can be used to find the longest route from each
...
### Combining *aggregate*, *union* and *filter* to compute distances {#aggrunion}

This next query is similar to the one we looked at in the [Finding all
...
### More queries that analyze distances {#_more_queries_that_analyze_distances}

Calculating the distance between two directly connected airports is very
...
### How far is it from AUS to LHR with one stop? {#AUSLHR-OS}

The next query begins to address the question \"How far is it from AUS
...
### Using *sack* to calculate the shortest AUS-LHR route with one stop {#sackauslhr}

Consider a typical use case for air travel. We want to calculate the
...
### Another example of how *sack* can be used {#anothersack}

The query below finds all airports with more than 200 routes and returns
...
## Using latitude, longitude and geographical region in queries {#latlonmanual}

The *air-routes* graph stores the geographic coordinates (latitude and
...
### Using the *math* step to calculate Great Circle distances {#GreatCircle}

If the latitude and longitude of two places on the Earth are known, the
...
## Finding routes that go via a specific airport {#via}

We have seen a number of examples that use a *has* step to filter
...
## Using *store* and a *sideEffect* to make a set of unique values {#storesideeffect}

Take a look at the query below. All it does is return the city names for
...
## Making a copy of the DFW vertex {#dfwcopy}
...
## Modelling an ordered binary tree as a graph {#btree}

You can of course model a tree structure as a graph. The following code
...
## Using *map* to produce a concatenated result string {#mapstr}

In the example below we use *map* to build a string containing the
...
## Randomly walking a graph {#randwalk}

When doing analysis of a graph sometimes you just want to randomly
...
## Seven degrees of separation! {#sevendegrees}
...
### Finding isolated sub-networks {#mininetworks}

As discussed above, while I was working on the queries for the previous
...
## Looking for the journey requiring the most stops {#longestroutes}

Sometimes it is interesting to ponder questions such as \"Starting from
...
### Quickly finding the hardest to get to airports {#quicklyfindhardest}

There are other ways we could write a query that looks for \"hard to get
...
## Finding unwanted parallel edges {#unwantededges}

In general terms there are many reasons in a graph that there could be
...
### Using *groupCount* with *path* to find duplicate edges {#groupcountpath}

The prior example found duplicate edges by working with a map created
...
## Finding the longest flight route between two adjacent airports in the graph {#flr}

This section is in a way a case study in the good and the bad of the
...
## Miscellaneous other queries {#miscq}

The examples in this section demonstrate a few miscellaneous features
...
### Using a calculation inside of an *is* step {#_using_a_calculation_inside_of_an_is_step}

It is possible to use a mathematical expression inside of an *is* step.
...
# MOVING BEYOND THE CONSOLE AND TINKERGRAPH {#beyond}

Most of the examples we have looked at so far were produced using the
...
## Working with TinkerGraph from a Java Application {#javatinker}

So far in this book we have looked at many ways of working with a
...
### The Apache TinkerPop interfaces and classes {#tpinterfaces}

There are a number of Java interfaces and classes, defined by the Apache
...
### Writing our first TinkerPop Java program {#javatp1}

Now that we have some imports in place, we can start to craft the basic
...
### Compiling our code {#javacompile}

Before we can test our program we of course need to compile it. The
...
### Adding to our Java program {#javaadd}

Now that we have a basic skeleton application that compiles and runs, we
...
### Important Classes and Enums to be aware of {#javastatics}

When you are using the Gremlin Console, as I have already mentioned, a
...
### Using Gremlin predicates in a Java application {#javapredicates}

As discussed in the previous section, when you use a Gremlin predicate
...
### Checking to see if a query returned a result {#javacheck}

It is often important to know if a query returned a result before trying
...
### Creating a new graph from a Java application {#javacreate}

The code below creates a new (empty) TinkerGraph instance, creates a
...
### Saving a graph from a Java application {#javasave}

Having created our new graph, we may want to save it. The code below
...
## Working with TinkerGraph from a Groovy application {#groovyapp}

Earlier in this book, in the \"[Making Gremlin even Groovier](#grv)\"
...
### Compiling our Groovy application {#groovyc}

In the Java example, I included the CLASSPATH on the *javac* invokation
...
### Running our Groovy application {#_running_our_groovy_application}

Assuming everything compiled cleanly we can now run our Groovy
application.
...
### Adding to our Groovy application {#_adding_to_our_groovy_application}

The code below demonstrates how to work with the *valueMap* for the
...
### Using Gremlin predicates in a Groovy application {#groovypredicates}

Just as we had to do when writing a standalone Java application, when
...
## Introducing JanusGraph {#janusintro}

So far we have been using the TinkerGraph graph that is included with
...
### Installing JanusGraph {#janusinstall}

JanusGraph itself is very easy to install. You just have to download the
...
### Using JanusGraph from the Gremlin Console {#janusconsole}

A version of the Gremlin Console is included as part of the JanusGraph
...
### Using JanusGraph with the *inmemory* option {#janusinmemory}

For almost all production use cases, you will be using JanusGraph along
...
### JanusGraph transactions {#janustrans}

So far we have been mainly using a TinkerGraph to perform our
...
### The JanusGraph management API {#janusmgmt}
...
### Creating a property with cardinality LIST {#cardlist}

Using the JanusGraph Management API it is possible to specify that a
...
### Creating a property with cardinality SET {#cardset}

Using the JanusGraph Management API we can also specify that a property
...
## Defining a JanusGraph schema for the air-routes graph {#janusschema}

You are not required to define the types and labels of your edges,
...
### Defining edge labels and usage {#_defining_edge_labels_and_usage}

Let's look at how we can use the JanusGraph Management API to specify
...
### Defining vertex labels {#_defining_vertex_labels}

Now let's tell JanusGraph about the vertex labels that we are going to
...
### Defining vertex property keys {#_defining_vertex_property_keys}

Next we need to define the property keys and data types that our
...
### Defining edge property keys {#_defining_edge_property_keys}

We also need to define the property keys and data types that will be
...
### Loading air-routes into a JanusGraph instance {#janusload}

Now that we know how to create a schema and an index for the
...
## JanusGraph indexes {#jaindexintro}

JanusGraph supports two different types of indexing known as *graph
...
If you have used other types of database such as a relational database,
...
### Vertex centric indexes {#vciintro}

A vertex centric index, as the name suggests is an index associated with
...
### Introducing *composite* indexes {#compositeintro}

A composite index can be used to speed up queries where an exact match
...
### Introducing *mixed* indexes {#mixedintro}

As mentioned above, if the queries that you expect to be writing require
...
### Building a composite index to speed up exact match searching {#compositeindex}

It is strongly recommended that you create graph indexes for any
...
### A script to automate schema creation, indexing and graph loading {#_a_script_to_automate_schema_creation_indexing_and_graph_loading}

In the sample code directory of my GitHub project for this book you will
...
## Additional JanusGraph text search predicates {#janpred}

We have already looked, in the \"[Testing values and ranges of
...
### Text comparison predicates {#_text_comparison_predicates}

The simplest of the search predicates allow you to specify an exact
...
### Regular expression predicates {#_regular_expression_predicates}

The JanusGraph regular expression predicates recognize the syntax
...
### Fuzzy search predicates {#_fuzzy_search_predicates}

These predicates use the [Levenshtein
...
## The JanusGraph GeoSpatial API {#janusgeo}
...
## Choosing a persistent storage technology for JanusGraph {#janusstorage}

So far we have concentrated on examples where the graph data resides in
...
### Oracle Berkley DB {#berkleyintro}

Oracle Berkely DB may be a good choice if your application runs on a
...
### Apache Cassandra {#cassandraintro}

If a single machine storage solution, such as that offered by Berkley
...
### ScyllaDB {#scyllaintro}

ScyllaDB is API compatible with Apache Cassandra but implemented in C++.
...
### Apache HBase {#hbaseintro}

If you already have a Hadoop and HDFS environment setup or are planning
...
### Google Bigtable {#_google_bigtable}

All of the options discussed so far are open source alternatives that
...
### IBM Compose for JanusGraph {#_ibm_compose_for_janusgraph}

IBM offers a hosted and managed JanusGraph environment via its Compose
...
### Other TinkerPop compatible products and services {#_other_tinkerpop_compatible_products_and_services}
...
## Using Docker to experiment with Cassandra and JanusGraph {#dockercass}

I find that using Docker containers can be a great way to quickly get
...
### Starting the Cassandra container {#cassstart}

Once Docker has downloaded the Cassandra image for you, it is quite
...
### Connecting JanusGraph to Cassandra {#cassconnect}

Now that we have an instance of a Cassandra running, it's time to start
...
## Using an external index with JanusGraph {#extindex}

JanusGraph allows an external index to be created using a technology
...
# INTRODUCING GREMLIN SERVER {#gremlinserver}

So far in this book we have looked at a few different ways to setup a
...
## Configuring Gremlin Server {#serverconfig}

The Gremlin Server runtime is a separate download available from the
...
## Connecting to a Gremlin Server from the Gremlin Console {#serverconsole}

It is fairly straightforward to connect to a running Gremlin Server from
...
### Making the remote connection {#remoteconn}

Now that we have our YAML file ready, all that we have to do to
...
### The Gremlin Console's *result* variable {#resultvar}

When working within the Gremlin console, one other useful thing to be
...
### Working in remote mode {#remotemode}

As useful as keeping the console in *\"local mode\"* can be, if you are
...
## Connecting to a Gremlin Server from the command line {#servercli}

Now that we have a Gremlin Server up and running that supports both HTTP
...
## Connecting to a Gremlin Server from Java using *withRemote* {#javagsclient}

While it is perfectly possible to work directly with the JSON returned
...
## Connecting to a Gremlin Server from Ruby {#rubyclient}

As far as I know, at time of writing, there is currently no formal
...
## Configuring a Gremlin Server to use a TinkerGraph {#servertinkergraph}

We have already seen how a Gremlin Server can be configured as a way to
...
### Creating the configuration files {#TGConfig}

To get our remote TinkerGraph up and running, all we have to do is to
...
### Starting the Server {#TGStart}

As discussed in the \"[Configuring Gremlin Server](#serverconfig)\"
...
### Testing the Server {#TGTest}

Now that the Gremlin Server is up and running you can access it using
...
## Tweaking queries to make the JSON returned easier to work with {#servertweaks}

Below is a query that we have seen used earlier in this book. It finds
...
## More examples of the JSON returned from a Gremlin Server {#serverjson}

The JSON returned by the Gremlin Server depends on query that is used
...
### No result {#_no_result}

The following query does not return any results. The JSON reflects this
...
### Integer result {#_integer_result}

A simple query that just returns a single integer result will generate
...
### String result {#_string_result}

As with integer results, a query that just returns a single string
...
### List of strings {#_list_of_strings}

The query below generates a *data* array containing a list of strings
...
### List of integers {#_list_of_integers}

The query below generates a *data* array containing a list of integers
...
### List of mixed types {#_list_of_mixed_types}

It is common for a query result to contain a variety of different data
...
### Value map {#_value_map}

As you might expect, when a *valueMap* is used to generate the result
...
When your query returns a vertex, unlike in the Gremlin Console where
...
### Selected vertex information {#_selected_vertex_information}

One way to limit the amount of JSON we get back is shown below. Let's
...
Just as when we queried a single vertex, when we query a single edge, we
...
### New vertex {#_new_vertex}

When a new vertex and some properties are added the returned JSON will
...
### New vertex only returning the ID {#_new_vertex_only_returning_the_id}

When adding a new vertex, if you are not really interested in getting
...
### Path by value (list of strings) {#_path_by_value_list_of_strings}

The query below returns a path between two airports as a list of airport
...
### Path by values (list of strings and integers) {#_path_by_values_list_of_strings_and_integers}

Similar to the previous query but this time the path also includes the
...
### Two vertex path {#_two_vertex_path}

The query below returns a path but does not include a *by* modulator so
...
### Path with two vertices and an edge {#_path_with_two_vertices_and_an_edge}

The following query is similar to the previous one but also includes an
...
### Selection map {#_selection_map}

If a query ends with a *select* step that references labels defined
...
### Projected map {#_projected_map}

The *project* step also generates a map just as the *select* step did in
...
### Strings and a map {#_strings_and_a_map}

The following path query returns a list containing two strings
...
### Nested lists {#_nested_lists}

When using the console or issuing Gremlin commands via the TinkerPop API
...
# COMMON GRAPH SERIALIZATION FORMATS {#serialize}

There are a number of ways that graph data can be stored in a file. In
...
There are a number of ways that a graph can be stored using CSV files.
...
### Using two CSV files to represent the air-routes data {#csvair}

If we were to store the airport data from the *air-routes* graph in CSV
...
### Adjacency matrix format {#_adjacency_matrix_format}

The examples shown above of how a CSV file can be used to store data
...
### Adjacency List format {#_adjacency_list_format}

The adjacency matrix shown above could also be represented as an
...
### Edge List format {#_edge_list_format}

When using an *edge list* format, each line represents an edge. So our
...
As discussed in the \"[Working with GraphML and
...
### Adjacency list format GraphSON {#_adjacency_list_format_graphson}
...
### Wrapped adjacency list format GraphSON {#_wrapped_adjacency_list_format_graphson}
...
Below you will find a selection of links to additional material related
...
## Additional Apache TinkerPop community links {#tplinks}
...
## Gremlin related mailing lists and discussion groups {#gremlindiscussionlists}
...
## JanusGraph links {#janusgraphlinks}
